...
lookup = data.frame(species = c("LUT GRIS", "LAC MAXI", "LUT ANAL", "OCY CHRY"),
...
test_that('returns unchanged data if group option not passed',
          expect_equal(species_group(ssu_density(ssu)), ssu_density(ssu)))
test_that('raises error if group is not a data.frame',
          expect_error(species_group(ssu_density(ssu), group = 1))
          )
test_that('returns correct density',
          {
            dens = species_group(ssu_density(ssu), group = lookup)
...
test_that('returns correct biomass',
          {
            biom = species_group(ssu_biomass(ssu, taxonomic_data), group = lookup)
