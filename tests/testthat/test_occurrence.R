...
test_that("ssu_occurrence output exactly matches baseline", {
  # Feed a larger subset to get multiple records
  ssu_input = subset(fk2012, PRIMARY_SAMPLE_UNIT == "005U")
  
  res = ssu_occurrence(ssu_input)
  expect_snapshot(res)
})

test_that("psu_occurrence output exactly matches baseline", {
  psu_input = subset(fk2012, STRAT == "MCPR")
  
  res = psu_occurrence(ssu_occurrence(psu_input))
  expect_snapshot(res)
})

test_that("strat_occurrence output exactly matches baseline", {
  strat_input = subset(fk2012, SPECIES_CD == "EPI MORI" & STRAT == "FSLR" & PROT == 0)
  
  res = strat_occurrence(psu_occurrence(ssu_occurrence(strat_input)), ntot2012)
  expect_snapshot(res)
})

test_that("domain_occurrence output exactly matches baseline", {
  domain_input = subset(fk2012, SPECIES_CD == "STE VARI")
  
  res = domain_occurrence(strat_occurrence(psu_occurrence(ssu_occurrence(domain_input)), ntot2012), ntot2012)
  expect_snapshot(res)
})
