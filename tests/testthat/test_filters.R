test_that("species_filter output exactly matches baseline", {
  species_data = data.frame(SPECIES_CD = c("EPI MORI", "MYC BONA"))
  
  expect_snapshot(species_filter(species_data, "EPI MORI"))
  expect_snapshot(species_filter(species_data, c("EPI MORI", "MYC BONA")))
  expect_snapshot(species_filter(species_data))
})

test_that("strata_filter output exactly matches baseline", {
  stratum_data = data.frame(STRAT = c("FDLR", "FSLR"))
  
  expect_snapshot(strata_filter(stratum_data, "FSLR"))
  expect_snapshot(strata_filter(stratum_data, c("FSLR", "FDLR")))
  expect_snapshot(strata_filter(stratum_data))
})

test_that("protected_filter output exactly matches baseline", {
  prot_data = data.frame(PROT = c(1, 0, 2))
  
  expect_snapshot(protected_filter(prot_data))
  expect_snapshot(protected_filter(prot_data, 0))
  expect_snapshot(protected_filter(prot_data, c(0,1)))
  expect_snapshot(protected_filter(prot_data, is_protected = TRUE))
  expect_snapshot(protected_filter(prot_data, is_protected = FALSE))
})

test_that("year_filter output exactly matches baseline", {
  year_data = data.frame(YEAR = c(2000, 2001))
  
  expect_snapshot(year_filter(year_data))
  expect_snapshot(year_filter(year_data, years = 2001))
})

test_that("region_filter output exactly matches baseline", {
  region_data = data.frame(REGION = c("FLA KEYS", "DRTO"))
  
  expect_snapshot(region_filter(region_data))
  expect_snapshot(region_filter(region_data, region = "FLA KEYS"))
})

test_that("length_filter output exactly matches baseline", {
  length_data = data.frame(LEN = c(0,1,2,3), NUM = c(1,2,3,4))
  
  expect_snapshot(length_filter(length_data))
  expect_snapshot(length_filter(length_data, len_geq = 2))
  expect_snapshot(length_filter(length_data, len_lt = 3))
  expect_snapshot(length_filter(length_data, len_lt = 3, len_geq = 2))
})

test_that("count_filter output exactly matches baseline", {
  count_data = data.frame(NUM = c(0, 0.25, 1, 3))
  
  expect_snapshot(count_filter(count_data))
  expect_snapshot(count_filter(count_data, cnt_geq = 2e-6))
  expect_snapshot(count_filter(count_data, cnt_lt = 1))
  expect_snapshot(count_filter(count_data, cnt_geq = 2e-6, cnt_lt = 3))
  expect_snapshot(count_filter(count_data, when_present = TRUE))
})
